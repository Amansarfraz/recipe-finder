...
@router.get("/mine")
async def get_my_recipes(current_user: dict = Depends(get_current_user)):
    """Lists the recipes the logged-in user has personally published."""
    cursor = recipes_collection.find({"created_by": current_user["_id"]}).sort("_id", -1)
    return [serialize_recipe(doc) async for doc in cursor]
