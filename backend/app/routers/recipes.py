...
async def _find_and_enrich(ing_list: list[str], max_ready_time: Optional[int] = None, number: int = 10):
    ...
    bulk info call. Filters by max_ready_time in Python afterwards,
    since findByIngredients doesn't support that filter directly.
    """
...
async def search_recipes(ingredients: str, diet_type: Optional[str] = "Any",
    ...
    # 1. Search user-submitted recipes (matches recipes containing ANY of the given ingredient names)
    ...
    # 2. Search Spoonacular — forgiving ingredient match
    external = []
    try:
        external = await _find_and_enrich(ing_list, max_ready_time=max_cook_time)
    except Exception:
        external = []  # fail gracefully if API key missing/quota hit
